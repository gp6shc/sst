...
/* 					if (true) { // force cache update */
						...
						$tweets = $connection->get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=".$instance['username']."&count=".$instance['tweetstoshow']."&exclude_replies=".$instance['excludereplies']) or die('Couldn\'t retrieve tweets! Wrong username?');
					...
					<div class="tp_recent_tweets">
						...
						foreach($tp_twitter_plugin_tweets as $tweet){					
							if(!empty($tweet['text'])){
								...
								print '<li><span>'.$tweet['text'].'</span><br /><a class="twitter_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.tp_relative_time($tweet['created_at']).'</a></li>';
...
if (!function_exists('tp_convert_links')) {
	function unshorten_url($url) {
		...
		if (!$link_unshorten_stats) {
			// initiate option if not previosly set
			$link_unshorten_stats = array(
				'expandurl.appspot.com' => array(
					'success' => 0,
					'fail' => 0
				),
				'api.longurl.org' => array(
					'success' => 0,
					'fail' => 0
				),
				't.co' => array(
			...
			$link_unshorten_stats = json_encode($link_unshorten_stats);
		...
		if (!$response || json_decode($response)->status !== 'OK') {
			// file_get_contents() returns false if connection fails; first API failed, try the next:
			$link_unshorten_stats->{'expandurl.appspot.com'}->fail++; // expandurl: fail +1
			
			$response = file_get_contents("http://api.longurl.org/v2/expand?format=json&url=".urlencode($url[0]));
			
			if ($response) {
				// this API didn't fail, lets go;
				$link_unshorten_stats->{'api.longurl.org'}->success++; // longurl: success +1
				
				$response = json_decode($response);
				$full_url = $response->{'long-url'};
				$display_url = preg_replace('#^https?://#', '', $full_url);
				$display_url = strlen($display_url) <= 30 ? $display_url : substr($display_url, 0, 30) . "…";
				$response = "<a class=\"ugly-url\" href=\"$full_url\" target=\"_blank\">$display_url</a>";
			}else{
				// all APIs have failed, stick with t.co :(
				$link_unshorten_stats->{'api.longurl.org'}->fail++; // longurl: fail +1
				$link_unshorten_stats->{'t.co'}->used++; // t.co: used +1
				$response = "<a class=\"ugly-url\" href=".$url[0]." target=\"_blank\">".$url[0]."</a>";
			}
		}else{
			// the first API responded, lets go:
			$link_unshorten_stats->{'expandurl.appspot.com'}->success++; // expandurl: success +1

			$response = json_decode($response);
			$full_url = $response->end_url;
			$display_url = preg_replace('#^https?://#', '', $full_url);
			$display_url = strlen($display_url) <= 30 ? $display_url : substr($display_url, 0, 30) . "…";
			$response = "<a class=\"ugly-url\" href=\"$full_url\" target=\"_blank\">$display_url</a>";
		}
		...
		// save stats
		$link_unshorten_stats = json_encode($link_unshorten_stats);
		update_option('link_unshorten_stats', $link_unshorten_stats);
		return $response;
	}
